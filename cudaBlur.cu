...
//Returns: None
__device__
void computeColumn(uint8_t * src, float * dest, int col,
    int pWidth, int height, int radius, int bpp)
{
    int i;
    ...
    // now shift everything up by radius spaces and blank out the last radius items to 
    // account for sums at the end of the kernel, instead of the middle
...
/////////////////////////////////////////////////////////////////////////////////
//                        Column Kernel Function                               //
/////////////////////////////////////////////////////////////////////////////////
__global__
void columnKernel(uint8_t * img, float * mid, int pWidth, int height, int radius, int bpp)
{
    // allocate to each thread
    int i;
    for (i = threadIdx.x; i < pWidth; i += blockDim.x)
    {
...
/////////////////////////////////////////////////////////////////////////////////
__global__
void rowKernel(float* mid, float* dest, int height, int pWidth, int radius, int bpp)
{
    // allocate to each thread
    int i;
    for (i = threadIdx.x; i < height; i += blockDim.x)
    {
...
int main(int argc, char** argv)
{
    ...
    //mid = (float *)malloc(sizeof(float) * pWidth * height);
    //dest = (float *)malloc(sizeof(float) * pWidth * height);

    uint8_t * gpu_img;

    cudaMallocManaged(&gpu_img, sizeof(uint8_t) * pWidth * height);
    memcpy(gpu_img, img, sizeof(uint8_t) * pWidth * height);
    ...
    // call kernal
    columnKernel << <1, 256 >> > (gpu_img, mid, pWidth, height, radius, bpp);
    ...
    // call kernal
    rowKernel << <1, 256 >> > (mid, dest, height, pWidth, radius, bpp);
    cudaDeviceSynchronize();
    cudaStatus = cudaGetLastError();
    assert(cudaStatus == cudaSuccess);
    ...
    //// copy data
    //cudaMemcpy(dest, gpu_dest, sizeof(float) *pWidth * height, cudaMemcpyDeviceToHost);
